...
import { Mail, MessageCircle, Phone, CreditCard, Send, DollarSign, Clock, ChevronLeft, ChevronRight } from "lucide-react";
...
export function ActivityFeed({ events, page = 1, hasMore = false }: { events: Record<string, unknown>[]; page?: number; hasMore?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-2 text-sm text-zinc-400">
        <div>Showing {events.length} activities</div>
        <div className="flex items-center gap-2">
          ...
          <Link href={`/activity?page=${page + 1}`}>
            <Button variant="secondary" size="sm" disabled={!hasMore} className="h-8 border-white/10 bg-transparent text-zinc-300 hover:bg-white/[0.05]">
