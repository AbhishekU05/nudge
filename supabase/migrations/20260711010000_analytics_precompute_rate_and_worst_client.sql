-- AnalyticsClient was still computing three things itself instead of just
-- rendering server-provided numbers:
--   - collectionRate (a division) — now stats.collection_rate.
--   - avgDaysOverdue (a division + rounding) — now stats.avg_days_overdue.
--   - worstClient — was derived by re-sorting topOffenders (which is capped
--     to the top 5 BY AMOUNT) to find the max by DAYS overdue. That silently
--     misses the true worst-by-days client whenever they aren't also in the
--     top 5 by money. Replaced with its own query over the full overdue
--     population, independent of the amount-based topOffenders cap.
