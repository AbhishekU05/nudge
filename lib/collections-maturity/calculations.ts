...
export type MaturityLevel = "Collections At Risk" | "Growing Agency" | "Operationally Strong" | "Top Quartile";
...
export type MaturityInputs = Record<string, number>;
...
export const SCALE_LABELS = ["Never", "Rarely", "Sometimes", "Usually", "Almost Always", "Always"];
...
const CATEGORY_ORDER: MaturityCategory[] = [
  "followUpDiscipline",
  "promiseTracking",
  "visibility",
  "automation",
...
export function calculateMaturity(answers: MaturityInputs): MaturityResults {
  const categoryScores: Record<MaturityCategory, number> = {
...
function getLevel(score: number): MaturityLevel {
  if (score >= 86) return "Top Quartile";
  if (score >= 71) return "Operationally Strong";
  if (score >= 41) return "Growing Agency";
...
export function getWeakestExplanation(key: MaturityCategory): string {
  switch (key) {
...
export function getStrongestExplanation(key: MaturityCategory): string {
  switch (key) {
    case "followUpDiscipline":
...
function getRecommendations(weakestCategory: MaturityCategory): string[] {
...
export function getDuelyPositioning(weakestCategory: MaturityCategory) {
  switch (weakestCategory) {
    case "followUpDiscipline":
      return {
