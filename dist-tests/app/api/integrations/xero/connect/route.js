...
async function GET() {
    ...
    try {
        const consentUrl = await (0, xero_1.buildXeroConsentUrl)(user.id);
        console.log("XERO CONSENT URL GENERATED:", consentUrl);
        return server_1.NextResponse.redirect(consentUrl);
    }
    catch (error) {
        return redirectToSettings("error", error instanceof Error ? error.message : "Unable to start Xero connection.");
    }
}
