...
exports.recordPartialPayment = recordPartialPayment;
exports.markFullyPaid = markFullyPaid;
exports.undoMarkAsPaid = undoMarkAsPaid;
...
exports.logFollowUp = logFollowUp;
exports.enableAutomation = enableAutomation;
exports.deletePaymentLog = deletePaymentLog;
exports.saveInternalNotes = saveInternalNotes;
exports.correctAmountPaid = correctAmountPaid;
...
const integrations_push_1 = require("@/lib/integrations-push");
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function redirectToNewCustomer(error) {
    (0, navigation_1.redirect)((0, paths_1.buildPathWithQuery)("/customers/new", { error }));
}
 ...
 */
async function insertPayment({ supabase, organizationId, invoiceId, amount, currency, xeroId, quickbooksId, }) {
    ...
    if (xeroId)
        (0, integrations_push_1.pushPaymentToXero)(organizationId, xeroId, amount, today);
    else if (quickbooksId)
        (0, integrations_push_1.pushPaymentToQuickBooks)(organizationId, quickbooksId, amount, today);
    return true;
}
async function logEvent({ supabase, organizationId, invoiceId, clientId, eventType, description, }) {
...
async function createCustomer(formData) {
    ...
    if (!organizationId)
        (0, navigation_1.redirect)("/settings/billing?error=subscription_required");
    const supabase = await (0, server_1.createSupabaseServerClient)();
    const clientId = getString(formData, "customer_id");
    if (!clientId)
        redirectToNewCustomer("Customer selection is required.");
    ...
    if (!amountInput)
        redirectToNewCustomer("Amount owed is required.");
    const amount = parseMoney(amountInput);
    if (amount === null)
        redirectToNewCustomer("Enter a valid amount (e.g. 420.00).");
    ...
    if (error) {
        logger_1.logger.error({ message: "Database error creating invoice", context: "createCustomer", user_id: user.id, error: error.message });
        redirectToNewCustomer("An unexpected error occurred. Please try again.");
    }
    logger_1.logger.action({ action_name: "create_invoice", user_id: user.id, success: true });
...
// ---------------------------------------------------------------------------
async function recordPartialPayment(formData) {
    const user = await (0, auth_1.requireUser)();
    try {
    ...
    if (typeof invoiceId !== "string" || !invoiceId)
        redirectToDashboard({ error: "Invalid invoice." });
    ...
    const newStatus = newRemaining <= 0 ? "paid" : newTotalPaid > 0 ? "partial" : "outstanding";
    const { error } = await supabase
        ...
        .eq("organization_id", organizationId);
    if (error) {
        logger_1.logger.error({ message: "Database error recording partial payment", context: "recordPartialPayment", user_id: user.id, error: error.message });
    ...
    if (!inserted)
        redirectToDashboard({ error: "Payment was recorded, but the payment history entry could not be saved." });
    await logEvent({ supabase, organizationId: organizationId, invoiceId: invoiceId, clientId: invoice.client_id, eventType: "status_change", description: `Payment of ${amount.toFixed(2)} ${invoice.currency} recorded.` });
    logger_1.logger.action({ action_name: "record_partial_payment", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
    redirectToDashboard({
...
// Mark fully paid
// ---------------------------------------------------------------------------
async function markFullyPaid(formData) {
    const user = await (0, auth_1.requireUser)();
    try {
        await (0, abuse_1.enforceRateLimit)(user.id, "reminder_toggle");
    }
    ...
    if (fetchError || !invoice)
        redirectToDashboard({ error: "Invoice not found." });
    const { data: paymentsData } = await supabase.from("payments").select("amount").eq("invoice_id", invoiceId);
    const totalPaid = (paymentsData ?? []).reduce((sum, p) => sum + Number(p.amount), 0);
    const remaining = Math.max(0, Number(invoice.amount) - totalPaid);
    const { error } = await supabase
        .from("invoices")
        .update({ status: "paid", reminders_enabled: false })
        .eq("id", invoiceId)
        .eq("organization_id", organizationId);
    if (error) {
        logger_1.logger.error({ message: "Database error marking paid", context: "markFullyPaid", user_id: user.id, error: error.message });
        redirectToDashboard({ error: "An unexpected error occurred." });
    }
    if (remaining > 0) {
        await insertPayment({
    ...
    await logEvent({ supabase, organizationId: organizationId, invoiceId: invoiceId, clientId: invoice.client_id, eventType: "status_change", description: "Invoice marked as fully paid." });
    logger_1.logger.action({ action_name: "mark_fully_paid", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
...
// Undo mark-as-paid
// ---------------------------------------------------------------------------
async function undoMarkAsPaid(formData) {
    const user = await (0, auth_1.requireUser)();
    try {
        await (0, abuse_1.enforceRateLimit)(user.id, "reminder_toggle");
    }
    ...
    const { error } = await supabase
        ...
        .eq("organization_id", organizationId);
    if (error) {
        logger_1.logger.error({ message: "Database error undoing paid status", context: "undoMarkAsPaid", user_id: user.id, error: error.message });
        redirectToDashboard({ error: "An unexpected error occurred." });
    }
    // Delete all payments for this invoice so the balance is clean
    await supabase.from("payments").delete().eq("invoice_id", invoiceId);
    logger_1.logger.action({ action_name: "undo_mark_paid", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
...
// ---------------------------------------------------------------------------
async function recordPaymentPromise(formData) {
    const user = await (0, auth_1.requireUser)();
    try {
        await (0, abuse_1.enforceRateLimit)(user.id, "reminder_toggle");
    }
    ...
    const notesValue = typeof promiseNotes === "string" && promiseNotes.trim().length > 0
        ...
        : null;
    const supabase = await (0, server_1.createSupabaseServerClient)();
    const { data: invoice } = await supabase.from("invoices").select("client_id").eq("id", invoiceId).eq("organization_id", organizationId).single();
    const { error } = await supabase
        .from("invoices")
        ...
        .eq("organization_id", organizationId);
    if (error) {
        logger_1.logger.error({ message: "Database error recording payment promise", context: "recordPaymentPromise", user_id: user.id, error: error.message });
        redirectToDashboard({ error: "An unexpected error occurred." });
    }
    await logEvent({
        supabase,
        ...
        eventType: "followup",
        description: `Payment promised by ${promisedDateRaw}.${notesValue ? ` Note: ${notesValue}` : ""}`,
    });
    logger_1.logger.action({ action_name: "record_payment_promise", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
...
// ---------------------------------------------------------------------------
async function updateWorkflowStatus(formData) {
    const user = await (0, auth_1.requireUser)();
    const organizationId = await getOrganizationId(user.id);
    if (!organizationId)
        redirectToDashboard({ error: "No organization found." });
    ...
    if (typeof invoiceId !== "string" || !invoiceId)
        redirectToDashboard({ error: "Invalid invoice." });
    const status = formData.get("workflow_status");
    const validStatuses = ["outstanding", "promised", "partial", "paid", "overdue", "written_off"];
    ...
    await logEvent({ supabase, organizationId: organizationId, invoiceId: invoiceId, eventType: "status_change", description: `Status changed to ${status}.` });
    (0, cache_1.revalidatePath)("/", "layout");
...
// Update client email
// ---------------------------------------------------------------------------
async function updateCustomerEmail(formData) {
    const user = await (0, auth_1.requireUser)();
    try {
        await (0, abuse_1.enforceRateLimit)(user.id, "reminder_toggle");
    }
    ...
    if (typeof invoiceId !== "string" || !invoiceId)
        redirectToDashboard({ error: "Invalid invoice." });
    const recipientEmail = formData.get("recipient_email")?.trim().toLowerCase() ?? "";
    ...
    if (!invoice?.client_id)
        redirectToDashboard({ error: "Could not locate client for this invoice." });
    const { error } = await supabase
        .from("clients")
        ...
        .eq("organization_id", organizationId);
    if (error) {
        logger_1.logger.error({ message: "Database error updating client email", context: "updateCustomerEmail", user_id: user.id, error: error.message });
        redirectToDashboard({ error: "Failed to update email." });
    }
    logger_1.logger.action({ action_name: "update_customer_email", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
...
async function deleteCustomer(formData) {
    ...
    const { error } = await supabase
        ...
        .eq("organization_id", organizationId);
    if (error) {
        logger_1.logger.error({ message: "Database error deleting invoice", context: "deleteCustomer", user_id: user.id, error: error.message });
        redirectToDashboard({ error: "Failed to delete customer." });
    }
    logger_1.logger.action({ action_name: "delete_invoice", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
...
// Update due date
// ---------------------------------------------------------------------------
async function updateDueDate(formData) {
    const user = await (0, auth_1.requireUser)();
    try {
        await (0, abuse_1.enforceRateLimit)(user.id, "reminder_toggle");
    }
    ...
    const { error } = await supabase
        ...
        .eq("organization_id", organizationId);
    if (error) {
        logger_1.logger.error({ message: "Database error updating due date", context: "updateDueDate", user_id: user.id, error: error.message });
        redirectToDashboard({ error: "An unexpected error occurred." });
    }
    logger_1.logger.action({ action_name: "update_due_date", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
...
// ---------------------------------------------------------------------------
async function logFollowUp(formData) {
    const user = await (0, auth_1.requireUser)();
    try {
        await (0, abuse_1.enforceRateLimit)(user.id, "reminder_toggle");
    }
    ...
    logger_1.logger.action({ action_name: "log_followup", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
...
// ---------------------------------------------------------------------------
async function enableAutomation(formData) {
    const user = await (0, auth_1.requireUser)();
    try {
        await (0, abuse_1.enforceRateLimit)(user.id, "reminder_toggle");
    }
    ...
    const { error } = await supabase
        .from("invoices")
        .update({ reminders_enabled: true, reminder_frequency_days: frequency, next_send_at: (0, reminder_schedule_1.computeFirstReminderSendAt)() })
        .eq("id", invoiceId)
        .eq("organization_id", organizationId);
    if (error) {
        logger_1.logger.error({ message: "Database error enabling automation", context: "enableAutomation", user_id: user.id, error: error.message });
        redirectToDashboard({ error: "An unexpected error occurred." });
    }
    logger_1.logger.action({ action_name: "enable_automation", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
    (0, cache_1.revalidatePath)(`/customers/${invoiceId}`);
}
// ---------------------------------------------------------------------------
// Delete a payment record
// ---------------------------------------------------------------------------
async function deletePaymentLog(formData) {
    const user = await (0, auth_1.requireUser)();
    ...
    const newStatus = totalPaid >= Number(invoice?.amount ?? 0) ? "paid" : totalPaid > 0 ? "partial" : "outstanding";
    await supabase.from("invoices").update({ status: newStatus }).eq("id", invoiceId).eq("organization_id", organizationId);
    logger_1.logger.action({ action_name: "delete_payment_log", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
...
// ---------------------------------------------------------------------------
async function saveInternalNotes(formData) {
    const user = await (0, auth_1.requireUser)();
    const organizationId = await getOrganizationId(user.id);
    if (!organizationId)
        redirectToDashboard({ error: "No organization found." });
...
// ---------------------------------------------------------------------------
async function correctAmountPaid(formData) {
    const user = await (0, auth_1.requireUser)();
    try {
        await (0, abuse_1.enforceRateLimit)(user.id, "reminder_toggle");
    }
    ...
    const newAmountPaid = parseMoney(newAmountInput);
    ...
    const paid = newAmountPaid;
    const newStatus = paid >= amountOwed ? "paid" : paid > 0 ? "partial" : "outstanding";
    // Delete existing payments and insert a corrected single payment
    await supabase.from("payments").delete().eq("invoice_id", invoiceId).eq("organization_id", organizationId);
    if (paid > 0) {
    ...
    const { error } = await supabase
        ...
        .eq("organization_id", organizationId);
    if (error) {
        logger_1.logger.error({ message: "Database error correcting amount paid", context: "correctAmountPaid", user_id: user.id, error: error.message });
        redirectToDashboard({ error: "An unexpected error occurred." });
    }
    logger_1.logger.action({ action_name: "correct_amount_paid", reminder_id: invoiceId, user_id: user.id, success: true });
    (0, cache_1.revalidatePath)("/", "layout");
    redirectToDashboard({
        success: newStatus === "paid"
