...
async function claimEntity(params) {
...
exports.sendReminders = client_1.inngest.createFunction({ id: "send-reminders", triggers: [{ cron: "0 * * * *" }] }, async ({ step }) => {
    const requestId = crypto_1.default.randomUUID();
    logger_1.logger.cron({ job_name: "send_reminders", status: "start", request_id: requestId });
    const supabase = (0, admin_1.createSupabaseAdminClient)();
    ...
    for (const entity of entities) {
        ...
        if (!(0, payments_1.isAutomationAndIntegrationAllowed)(orgInfo.status, orgInfo.createdAt)) {
        ...
        try {
            ...
            if (entity.auto_approve) {
                const gmailAvailable = await (0, gmail_1.hasGmailTokens)(sender.user_id);
                if (gmailAvailable) {
                    try {
            ...
            if (entity.reminder_type === "sequence") {
                if (nextSequenceIndex < templates.length - 1) {
                    ...
                    nextSendAt = (0, reminder_schedule_1.computeRecurringReminderSendAt)(nextOffset, sentOrDraftedAt);
                }
                else {
                    await supabase
                        .from(table)
                ...
                nextSendAt = (0, reminder_schedule_1.computeRecurringReminderSendAt)(entity.reminder_frequency_days, sentOrDraftedAt);
            }
            await supabase
                .from(table)
                .update({
    ...
    logger_1.logger.cron({ job_name: "send_reminders", status: "end", processed: entities.length, success_count: sent + drafted, failure_count: failed, request_id: requestId });
