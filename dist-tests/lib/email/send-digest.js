...
exports.sendWeeklyDigestEmails = sendWeeklyDigestEmails;
require("server-only");
...
const WeeklyDigest_1 = __importDefault(require("@/components/emails/WeeklyDigest"));
async function sendWeeklyDigestEmails(targetUserId) {
    ...
    if (profilesError || !profiles) {
        logger_1.logger.error({ message: "Failed to fetch profiles for digest", error: profilesError, context: "sendWeeklyDigestEmails" });
    ...
    const eligibleProfiles = profiles.filter(profile => {
        if (targetUserId)
            return true; // manual trigger bypasses time check
        try {
    ...
    if (usersError || !usersData) {
        logger_1.logger.error({ message: "Failed to fetch users for digest", error: usersError, context: "sendWeeklyDigestEmails" });
    ...
    for (const profile of eligibleProfiles) {
        ...
        for (const currency of allCurrencies) {
            ...
            const upcomingInvoicesList = [];
            const promisesThisWeekList = [];
            const paymentsReceivedList = [];
            const actionItemsList = [];
            const agingBuckets = { "1-30": 0, "31-60": 0, "61-90": 0, "90+": 0 };
            const forecastBuckets = { "0-30 Days": 0, "31-60 Days": 0, "61-90 Days": 0 };
            ...
            currInvoices.forEach((inv) => {
                ...
                if (remaining <= 0)
                    return;
                const daysOverdue = (0, types_1.getDaysOverdue)(inv);
                if (daysOverdue !== null) {
                    totalOver += remaining;
                    overdueCount++;
                    if (daysOverdue <= 30)
            ...
            // Build charts
            const encodeChart = (config) => `https://quickchart.io/chart?c=${encodeURIComponent(JSON.stringify(config))}&w=500&h=300&bkg=transparent&f=png&v=3`;
        ...
        try {
            const htmlContent = await (0, render_1.render)((0, WeeklyDigest_1.default)({
                dateRange,
                currencies: currencyDigests
            }));
            const subject = "Your weekly collections snapshot";
            const hasGmail = await (0, gmail_1.hasGmailTokens)(userId);
            if (hasGmail) {
            ...
            logger_1.logger.error({ message: "Failed to send digest email", error: e, user_id: userId, context: "sendWeeklyDigestEmails" });
        }
    }
