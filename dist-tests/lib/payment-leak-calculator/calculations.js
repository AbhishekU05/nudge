...
exports.calculatePaymentLeak = calculatePaymentLeak;
...
function calculatePaymentLeak(inputs) {
    ...
    const cashTiedUp = activeClients *
        averageInvoiceValue *
    ...
    const clientConcentrationScore = activeClients >= 20 ? 0 : ((20 - activeClients) / 19) * 20;
    const riskScore = Math.round(clamp(latePaymentScore + delayDaysScore + clientConcentrationScore, 0, 100));
    const riskLevel = getRiskLevel(riskScore);
    const monthlyOperatingExpenses = inputs.monthlyOperatingExpenses && inputs.monthlyOperatingExpenses > 0
