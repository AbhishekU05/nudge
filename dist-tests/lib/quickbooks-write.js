...
exports.createQuickBooksLateFeeInvoice = createQuickBooksLateFeeInvoice;
exports.updateQuickBooksInvoiceWithLateFee = updateQuickBooksInvoiceWithLateFee;
require("server-only");
...
async function createQuickBooksLateFeeInvoice(organizationId, originalInvoiceNumber, feeAmount, contactName, email) {
    ...
    if (!customerRef)
        return null; // We can't create an invoice without a valid customerRef in QB
    const newInvoice = {
        CustomerRef: {
            value: customerRef
        },
...
async function updateQuickBooksInvoiceWithLateFee(organizationId, invoiceId, feeAmount) {
    const supabase = (0, admin_1.createSupabaseAdminClient)();
    const { data: integration } = await supabase.from("integrations").select("*").eq("organization_id", organizationId).eq("provider", "quickbooks").single();
    if (!integration || !integration.realm_id)
        return null;
