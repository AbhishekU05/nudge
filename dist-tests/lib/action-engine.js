...
function evaluateClient(client, invoices, totalUserAR) {
    const activeInvoices = invoices.filter((inv) => inv.workflow_status !== "paid" &&
        inv.workflow_status !== "written_off" &&
        (0, types_1.getRemainingBalance)(inv) > 0);
    ...
    for (const inv of activeInvoices) {
        clientTotalOwed += (0, types_1.getRemainingBalance)(inv);
    ...
    let score = 0;
    // A. Aging (Max 50)
    if (maxDaysOverdue >= 31)
        score += 50;
    else if (maxDaysOverdue >= 15)
        score += 40;
    else if (maxDaysOverdue >= 4)
        score += 25;
    else if (maxDaysOverdue >= 1)
        score += 10;
    else
    ...
    let contextText = "";
    let recommendation = "friendly_checkin";
    if (lastContactDaysAgo !== null) {
        let requiredCooldown = 0;
        if (maxDaysOverdue >= 31)
            requiredCooldown = 1;
        else if (maxDaysOverdue >= 11)
            requiredCooldown = 3;
        else
            requiredCooldown = 5;
...
function generateActionPlan(clients, allInvoices) {
    const activeInvoices = allInvoices.filter((inv) => inv.workflow_status !== "paid" &&
        inv.workflow_status !== "written_off" &&
        (0, types_1.getRemainingBalance)(inv) > 0 &&
        (0, types_1.getDaysOverdue)(inv) !== null);
    const totalUserAR = activeInvoices.reduce((sum, inv) => sum + (0, types_1.getRemainingBalance)(inv), 0);
    const tasks = [];
